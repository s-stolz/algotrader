...
def get_symbol_mapping(symbols: list[str]) -> Dict[str, int]:
    ...
    if missing_symbols:
        logger.info("Symbols not found in cache: %s", missing_symbols)
        ...
        if missing_symbols_mapping:
            ...
            logger.info("Updated SYMBOL_MAPPING with %d new entries", len(missing_symbols_mapping))
        else:
            logger.info("No new symbols found to update SYMBOL_MAPPING")
...
def get_symbol_id_sync(symbols: list[str]) -> dict[str, int]:
    ...
    for sym in symbols:
        ...
        if sid is not None:
            ...
            missing.append(sym)
    if missing:
        logger.info("Symbols not found (skipped): %s", missing)
    return mapping
...
def _fetch_symbol_id_sync(symbol: str) -> tuple[str, int | None]:
    ...
    try:
        ...
        if markets:
            ...
            logger.debug("Found ID %s for symbol %s", symbol_id, symbol)
            return symbol, symbol_id
        else:
            logger.debug("No market found for symbol: %s", symbol)
            return symbol, None
    except Exception as e:  # broad catch acceptable for I/O boundary
        logger.warning("Error fetching symbol ID for %s: %s", symbol, e)
        return symbol, None
